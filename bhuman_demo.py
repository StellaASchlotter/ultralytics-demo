...
#dataset = LoadImages("/work/input_images", img_size=detector.imgsz, stride=detector.model.stride, auto=detector.model.pt)
im0 = cv2.imread("/work/input_images/test.jpg")
im = letterbox(im0, detector.imgsz, stride=detector.model.stride, auto=True)[0]  # padded resize
im = im.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
im = np.ascontiguousarray(im)
detections = detector.run(im, im0.shape)
#for preprocessed_image, original_image in dataset:
